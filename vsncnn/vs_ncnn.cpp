...
static void VS_CC vsNcnnFree(
    ...
    for (auto & resource : d->resources) {
        // Destroy every object using these allocators before returning them to
        // the device pool, where another filter instance may acquire them.
        auto *blob = resource.blob_vkallocator;
        auto *staging = resource.staging_vkallocator;
        resource = Resource{};
...
static void VS_CC vsNcnnCreate(
    ...
    auto set_error = [&](const std::string & error_message) {
        ...
        // A partially initialized Net may own Vulkan objects too.
        d.reset();
        if (--num_plugin_instances == 0) {
            ncnn::destroy_gpu_instance();
        }
