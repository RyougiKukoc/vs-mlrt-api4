...
The shared model payload is published once on the separate `models` release tag as `models.zip`. VCS installs always download this shared model asset in addition to the selected CUDA tag. Set `VSMLRT_MODELS_TAG` or `VSMLRT_MODELS_RELEASE_REPO` only when testing a temporary model release.
